...
impl OpenAIProvider {
    ...
    fn build_request_body(&self, request: &LLMRequest) -> serde_json::Value {
        ...
        for m in &request.messages {
            ...
            // OpenAI messages can have mixed content types
        ...
        // max_tokens is called max_completion_tokens for newer models
        ...
        // Add stream options for streaming to get usage stats
        if request.stream {
    ...
    fn convert_content_blocks(&self, content: &[ContentBlock]) -> serde_json::Value {
        // OpenAI can accept either a string or an array of content parts
        let parts: Vec<serde_json::Value> = content
            ...
            .filter_map(|c| match c {
                ...
                // Tool use and tool results handled separately
                _ => None,
            })
            .collect();
...
impl Provider for OpenAIProvider {
    ...
    async fn complete(&self, request: LLMRequest) -> Result<LLMResponse, LLMError> {
        ...
        let resp: OpenAIResponse = response.json().await.map_err(|e| LLMError::ParseError {
            message: e.to_string(),
        })?;
    ...
    if let Some(choices) = json["choices"].as_array() {
        for choice in choices {
            ...
            if let Some(content) = delta["content"].as_str() {
                if !content.is_empty() {
                    // If we were in a tool block, we're back to text
                    if *in_tool_block {
                        *in_tool_block = false;
                        chunks.push(StreamChunk::BlockStop { index: *block_index - 1 });
                        chunks.push(StreamChunk::BlockStart {
...
fn convert_response(resp: OpenAIResponse) -> LLMResponse {
...
mod tests {
    ...
    fn test_parse_sse_text_delta() {
        ...
        // Should have Start, BlockStart, and Delta
        assert!(chunks.len() >= 2);
